...
public class S3FileStorage implements FileStorage {
    ...
    @Override
    public Optional<FileResponse> getFile(String folder, String fileName) throws IOException {
        ...
        if (client.doesObjectExist(folder, fileName)) {
            try (S3Object item = client.getObject(new GetObjectRequest(folder, fileName))) {
                file = of(new FileResponse(fileName, IOUtils.toByteArray(item.getObjectContent())));
            }
        }
        return file;
    }
